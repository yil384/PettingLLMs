...
class UnitTestGenerationAgent(Agent):
    ...
    def update_from_env(self, turn_idx: int, env_data: Env):
        ...
        # Extract ground truth test examples
        ground_truth_test_input = getattr(state, "ground_truth_test_input", None) or []
        ground_truth_test_output = getattr(state, "ground_truth_test_output", None) or []
        
        # Build example format from first ground truth test case if available
        example_text = ""
        if ground_truth_test_input and ground_truth_test_output and len(ground_truth_test_input) > 0 and len(ground_truth_test_output) > 0:
            first_input = ground_truth_test_input[0]
            first_output = ground_truth_test_output[0]
            example_text = (
                f"Here is one existing test case as a reference example:\n\n"
                f"**Example Test Case:**\n"
                f"**Test Input:**\n```{first_input}```\n\n"
                f"**Test Output:**\n```{first_output}```\n\n"
                f"Note: This is just a reference example. You need to create a NEW and DIFFERENT test case, not copy this one.\n\n"
            )
