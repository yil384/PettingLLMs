...
from pettingllms.env import REGISTERED_ENV_CONFIGS
...
def generate_initial_request(env_tag, config) -> str:
    env_config_new = config.custom_envs[env_tag]
    ...
    if env_config_new.get("action_lookup", False):
                ...
                action_lookup_str += f"\nYou can make up to {env_config_new['max_actions_per_traj']} actions, separated by the action separator \" " + config.action_sep + " \"\n"
